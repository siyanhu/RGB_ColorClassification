...
def responseOfNeighbors(neighbors):
    ...
    all_possible_neighbors_confidence = {}
    neighbour_content = list()
    neighbour_confidence = list()
    for i in range(len(neighbors)):
        neighbour_content.append(neighbors[i][0])
        neighbour_confidence.append(neighbors[i][1])
    for x in range(len(neighbour_content)):
        response = neighbour_content[x][-1]
        confidence = neighbour_confidence[x]
        if response in all_possible_neighbors:
            all_possible_neighbors[response] += 1
            all_possible_neighbors_confidence[response] += confidence
        else:
            all_possible_neighbors[response] = 1
            all_possible_neighbors_confidence[response] = confidence
    sortedVotes = sorted(all_possible_neighbors.items(), key=operator.itemgetter(1), reverse=True)
    target_neighbour = sortedVotes[0][0]
    denominator = all_possible_neighbors[target_neighbour]
    score = all_possible_neighbors_confidence[target_neighbour]
    target_confidence = float(score/denominator)
    return (target_neighbour, target_confidence)
    # all_possible_neighbors = {}
